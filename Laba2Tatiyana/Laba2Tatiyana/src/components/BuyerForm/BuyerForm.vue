<template>
  <div :class="$style.root">
    ...
    <div :class="$style.item">
      <div :class="$style.label">
        <label for="buyer">Марка автомобиля</label>
      </div>
    ...
    <div :class="$style.item">
      <div :class="$style.label">
        <label for="description">Впечатление о покупке</label>
      </div>
...
import { selectItemById, fetchItems, selectItems } from '@/store/buyers/selectors';
...
export default {
  ...
  setup(props, context) {
    ...
    const buyerList = computed(() => selectItems(store));
    const form = reactive({
      id: '',
      buyer: '',
      description: '',
    ...
    watchEffect(() => {
      const buyer = selectItemById( store,  props.id );
    ...
    return {
      buyerList,
      form,
      isValidForm: computed(() =>  !!(form.buyer && form.description)),
      onClick: () => {
