...
export default {
  ...
  actions: {
    fetchItems: async ({ commit }) => {
      const response = await api.buyers();
    ...
    addItem: async ({ commit }, { buyer, description }) => {
      const item = await api.add({ buyer, description });
      commit('addItem', item);
    },
    updateItem: async ({ commit }, { id, buyer, description }) => {
      const item = await api.update({ id, buyer, description });
